...
handler.before = async function (m, { conn, participants, groupMetadata }) {
  ...
  let nombre = `❀ *Cambio de Nombre*\n➪ Usuario › ${usuario}\n> Nuevo nombre › *${m.messageStubParameters[0]}*`
  let foto = `❀ *Cambio de Imagen*\n➪ Realizado por › ${usuario}`
  let edit = `❀ *Configuración del grupo actualizada*\n➪ Ahora › ${m.messageStubParameters[0] == 'on' ? 'solo admins' : 'todos'} pueden cambiar ajustes.`
  let newlink = `❀ *Enlace del grupo restablecido*\n➪ Acción › ${usuario}`
  ...
  let admingp = `❀ *Nuevo Administrador*\n➪ ${m.messageStubParameters[0]} ahora es admin\n> Acción de › ${usuario}`
  let noadmingp = `❀ *Admin Removido*\n➪ ${m.messageStubParameters[0]} dejó de ser admin\n> Acción de › ${usuario}`
